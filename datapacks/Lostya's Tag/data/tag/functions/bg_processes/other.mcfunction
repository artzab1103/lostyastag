...
scoreboard players set @a[tag=!tagger, scores={tagger_timer=1..}] tagger_timer 0
...
execute if score glow event matches 2000.. run title @a[tag = !tagger] title {"text":"Подсветка!","color":"#00FFFF"}
execute if score glow event matches 2000.. run title @a[tag = tagger] title {"text":"Подсветка!","color":"#00FFFF"}
execute if score glow event matches 2000.. run title @a[tag = !tagger] subtitle {"translate":"effect.glow","color":"#00FFFF"}
execute if score glow event matches 2000.. run title @a[tag = tagger] subtitle {"translate":"effect.glow","color":"#00FFFF"}
...
execute as @a[tag=afk, gamemode=adventure] unless score @s effect.invis matches 1.. at @s run particle dust 1 1 0 1 ~ ~1 ~ .2 .4 .2 0 3
...
# compute speeds
execute as @a[scores={walk2=0..}, nbt={OnGround:1b}] run scoreboard players operation @s stat.speed2 = @s walk2
execute as @a[scores={walk2=1..}] run scoreboard players set @s walk2 0
